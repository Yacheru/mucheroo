...
module.exports = {
	...
	async execute(interaction) {
		...
		if (member.isCommunicationDisabled()) {
			member.timeout(null);
			interaction.reply({ content: `C участника ${userMention(member.id)} успешно снято наказание по причине ${reason}`, ephemeral: true });
		}
		else {
			interaction.reply({ content: `Участник ${userMention(member.id)} не имеет Тайм-Аута.`, ephemeral: true });
		}
	},
};
